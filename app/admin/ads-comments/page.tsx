...
export default function AdminShowsPage() {
    ...
    const fetchVisitors = async () => {
        try {
            const response = await fetch("/api/admin/ads-comments");
            if (response.ok) {
            ...
            <main className="flex-1 overflow-auto">
                <div className="p-8">
                    <div className="flex justify-between items-center mb-8">
                        <h1 className="text-md md:text-4xl font-bold">Ads Comments</h1>
                    </div>
                            ...
                            visitors.map((show: any) => (
                                ...
                                >
                                    <div>
                                        ...
                                        <p className="text-sm text-muted-foreground">
                                            <strong>Message:</strong> {show.message}
                                        </p>

                                        <strong>Date:</strong> {formatDate(new Date(String(show.created_at)))}
                                    </div>
                                </Card>
                            ))
                        )}
