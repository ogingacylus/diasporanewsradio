...
export default function AdminShowsPage() {
  ...
  const handleDelete = async (id: number, item: any) => {
    ...
    if (!confirm("Are you sure?")) return;
    try {
      const response = await fetch(`/api/admin/marketing/${id}`, {
        method: "DELETE",
      });
      if (response.ok) {
        fetchMarketing();
        ...
        <div className="p-8">
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              <p className="text-muted-foreground">No marketing items yet</p>
            ) : (
              marketing.map((show: any, index: number) => (
                <Card
                  key={show.id}
