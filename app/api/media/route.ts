...
export async function GET() {
  try {
    ...
    const news =
      await sql`SELECT * FROM media WHERE published = true ORDER BY created_at DESC LIMIT 50
    `;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await sql`
      INSERT INTO media (title, published, 
      created_at) VALUES (${title}, ${published}, ${timeNow} )
    `;
